...
const Header = () => {
  ...
  return (
    <>
      <div
        className={`fixed top-0 left-0 w-64 h-full bg-white flex flex-col items-center z-[1000] shadow-lg transform transition-transform duration-300 ${isShowed ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        ...
        <div className="flex flex-col gap-4 py-8 w-[80%]">
          ...
          <div>
            ...
            >
              Shop
              <svg
                className={`w-4 h-4 ml-2 transition-transform duration-300 ${isShopVisible ? "rotate-180" : "rotate-0"
                  }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
        ...
        <div className="flex flex-col gap-4 mt-auto mb-5 pt-4 border-t border-neutral-300 w-[80%]">
          <span className="hover:text-amber-600 cursor-pointer" onClick={() => Nav("/signin")}>Login</span>
          <span className="hover:text-amber-600 cursor-pointer" onClick={() => Nav("/signup")}>
            Create Account
          </span>
        </div>
      </div>
      ...
      <header
        className={`py-5 sticky top-0 z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-sm" : "bg-white/80 backdrop-blur-sm"
          }`}
      >
        <div className="w-full max-w-[1500px] mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden lg:flex items-center space-x-8 text-[16px] font-medium relative">
            ...
            >
              ...
              <div
                className={`absolute top-full left-0 mt-2 w-40 bg-white shadow-md rounded-md py-2 transition-all duration-300 ${isShopVisible ? "opacity-100 visible" : "opacity-0 invisible"
                  }`}
              >
      ...
      <div
        className={`fixed top-0 left-0 right-0 h-40 bg-white shadow-lg z-50 transition-transform duration-300 ${isSearchVisible ? "translate-y-0" : "-translate-y-full"
          }`}
      >
